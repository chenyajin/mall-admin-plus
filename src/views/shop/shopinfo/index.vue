 ...
 * @LastEditors: 陈亚金
 * @LastEditTime: 2023-12-27 17:22:43
 * @Description: 门店列表
-->
<template>
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getShopInfo(id).then(response => {
        ...
        this.businessHour = response.data.openTime ? [new Date('January 12,2006 ' + response.data.openingTime), new Date('January 12,2006 ' + response.data.closingTime)] : []
        that.open = true;
        that.title = "修改门店";
    ...
    /** 提交按钮 */
    submitForm () {
      const baseShopItemConsultantList = this.itemIds.filter(item => !!item).map(item => {
        return {
          itemId: item,
          consultantId: ''
        }
      ...
      if (this.businessHour.length) {
        this.form.closingTime = moment(this.form.businessHour[0]).format('HH:mm:ss')
        this.form.openingTime = moment(this.form.businessHour[1]).format('HH:mm:ss')
      }
